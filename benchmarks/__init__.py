"""Fault Case loader and registry."""
...
# Registered fault handlers live in reference service / sandbox injectors.
...
class FaultCase(BaseModel):
    ...
    def validate_registered(self) -> None:
        if self.fault_type not in REGISTERED_FAULT_TYPES:
            raise ValueError(
                f"fault_type '{self.fault_type}' is not registered; "
                "YAML cannot introduce arbitrary shell commands"
            )
...
def load_fault_cases(directory: str | Path) -> list[FaultCase]:
    ...
    for path in sorted(directory.glob("*.yaml")):
        cases.append(load_fault_case(path))
    return cases
